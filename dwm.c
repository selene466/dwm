...
focus(Client *c)
{
	...
	selmon->sel = c;
    if (selmon->lt[selmon->sellt]->arrange == monocle) arrangemon(selmon);
	drawbars();
}
...
quit(const Arg *arg)
{
    ...
    system("pgrep autostart.sh | xargs kill");
	if(arg->i) restart = 1;
	running = 0;
}
...
showhide(Client *c)
{
	...
	if (ISVISIBLE(c)) {
		...
		/* 	/1* c->x = c->mon->wx + (c->mon->ww / 2 - WIDTH(c) / 2); *1/ */
		/* 	/1* c->y = c->mon->wy + (c->mon->wh / 2 - HEIGHT(c) / 2); *1/ */
		/* } */
